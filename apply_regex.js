...
const regex = /listEl\.innerHTML = notifications\.map\(item => \{[\s\S]*?\}\)\.join\(''\);\s*\};\s*window\.clearAdminNotifs = function\(\)/;

const replacement = `listEl.innerHTML = notifications.map(item => {
